...
(function (scenes) {
    ...
    var Play3 = (function (_super) {
        ...
        Play3.prototype.addLight = function () {
        ...
        Play3.prototype.addWalls = function () {
            //wall material
        ...
        Play3.prototype.addStartPlatform = function () {
            //startPlatform
        ...
        Play3.prototype.addEndPlatform = function () {
            //end
        ...
        Play3.prototype.addBlocks = function () {
            var self = this;
            this.blocks = new Array();
            for (var i = 0; i < self.blockCount; i++) {
                var x = Math.random() * 10 + 1;
                var z = Math.random() * 10 + 1;
                self.blocks[i] = new Physijs.ConvexMesh(new BoxGeometry(x, 1, z), Physijs.createMaterial(new LambertMaterial()), 1);
        ...
        Play3.prototype._unpauseSimulation = function () {
        ...
        Play3.prototype.update = function () {
            ...
            //send projectile after time interval
            this.ballTimer -= 1;
            if (this.ballTimer <= 0) {
                this.sendBall();
                this.ballTimer = 250;
            ...
            this.blocks.forEach(function (block) {
                //random x velocity between 4 and 1
                var vel = Math.random() * (4 - 1) + 1;
                //set velocity at the start of the level
                if (block.getLinearVelocity().x == 0) {
                    block.setLinearFactor(new Vector3(0, 0, 0));
                    block.setLinearVelocity(new Vector3(vel, 0, 0));
                }
                //change velocity when block reaches certain position
                if (block.position.x >= 10) {
                    block.setLinearFactor(new Vector3(0, 0, 0));
                    block.setLinearVelocity(new Vector3(-vel, 0, 0));
                }
                else if (block.position.x <= -10) {
                    block.setLinearFactor(new Vector3(0, 0, 0));
                    block.setLinearVelocity(new Vector3(vel, 0, 0));
                }
                block.setAngularFactor(new Vector3(0, 0, 0));
                block.setAngularVelocity(new Vector3(0, 0, 0));
            });
            this.checkControls();
            this.stage.update();
